...
- [x] **Observability + graceful shutdown (protocol level)** — structured
      ...
      benign peer disconnects (reset / broken pipe / EOF) logged calmly instead
      of as faults. The read loop logs its teardown rather than swallowing.
- [ ] **Observability (remaining)** — metrics/counters, and OS-signal-driven
      graceful shutdown (Eio 1.3 has no clean signal-receive API, so SIGINT
      still terminates rather than draining connections).
- [x] **Noise XX cross-impl vector** — the full handshake (msg1/2/3, channel
      binding, first transport message) is pinned byte-for-byte to a vector
      generated by **flynn/noise** (go-libp2p's Noise library); regenerate with
      `interop/starling-interop noisekat`. Offline counterpart to the live interop.
- [x] **Constant-time review** — done (see [`SECURITY.md`](SECURITY.md)): all
      secret-dependent operations (AEAD tag check, Ed25519 verify, X25519) are
      delegated to mirage-crypto; starling does no hand-rolled variable-time
      comparison on secret/tag/signature material.
- [ ] **Secret zeroization** — a known limitation, documented in `SECURITY.md`:
      not achievable at the application layer under OCaml's immutable strings +
      opaque crypto types + moving GC. Needs wipeable secret types in the
      underlying crypto library, not app-level wipe calls.
